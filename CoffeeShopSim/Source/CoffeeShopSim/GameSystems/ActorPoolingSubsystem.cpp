...
#include UE_INLINE_GENERATED_CPP_BY_NAME(ActorPoolingSubsystem)

static TAutoConsoleVariable CVarActorPoolingEnabled(
	TEXT("game.ActorPooling"),
	true,  // enabled pooling by default
...
void UActorPoolingSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	UE_LOGFMT(LogCoffeeShopSim, Log, "UActorPoolingSubsystem Initialize!");
}

void UActorPoolingSubsystem::Deinitialize()
{
	UE_LOGFMT(LogCoffeeShopSim, Log, "UActorPoolingSubsystem Deinitialize!");
...
AActor* UActorPoolingSubsystem::AcquireFromPool(const UObject* WorldContextObject, TSubclassOf<AActor> ActorClass, const FTransform& SpawnTransform, FActorSpawnParameters SpawnParams)
{
	...
	{
		UActorPoolingSubsystem* PoolingSystem = WorldContextObject->GetWorld()->GetSubsystem<UActorPoolingSubsystem>();
...
{
	UE_LOGFMT(LogCoffeeShopSim, Log, "Priming Pool for {actorclass} ({amount})", GetNameSafe(ActorClass), Amount);
...
AActor* UActorPoolingSubsystem::AcquireFromPool_Internal(TSubclassOf<AActor> ActorClass, const FTransform& SpawnTransform, FActorSpawnParameters SpawnParams)
{
	...
	// Adds the actor if it doesn't exist
	//  The ampersand (&) is used to get the reference to the value associated with the key ActorClass in the map.
	//  By using &, you can modify the value directly instead of working with a copy of the value.
	FActorPool* ActorPool = &AvailableActorPool.FindOrAdd(ActorClass);
	// Grab first available
	if (ActorPool->FreeActors.IsValidIndex(0))
	{
		UE_LOGFMT(LogCoffeeShopSim, Log, "Acquired Actor for {actorclass} from pool", GetNameSafe(ActorClass));
	...
	{
		UE_LOGFMT(LogCoffeeShopSim, Log, "Actor Pool empty, spawning new Actor for {actorclass}", GetNameSafe(ActorClass));
